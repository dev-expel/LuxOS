...
_x86_Video_WriteCharTeletype:
    ...
    ; [bp + 0] - old call frame
    ; [bp + 2] - return addr (small mem model => 2 bytes)
    ; [bp + 4] - first arg (char)
    ; [bp + 6] - second arg (page)
    ; note: bytes are converted to words (you can't push a byte on the stack)
    mov ah, 0Eh
    mov al, [bp + 4]
    mov bh, [bp + 6]
...
_x86_Disk_Read:
    ...
    mov cl, [bp + 7]        ; cl - cyldiner (bits 6-7)
    shl cl, 6
    ...
    mov ax, 1
    sbb ax, 0               ; 1 on success, 0 on fall
...
_x86_Disk_GetDriveParams:
    ...
    mov bl, ch              ; cylinder - lower bits in ch
    mov bh, cl              ; cyldiner - upper bits in cl (6-7)
    shr bh, 6
    mov si, [bp + 8]
    mov [si], bx
