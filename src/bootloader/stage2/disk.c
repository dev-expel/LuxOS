...
bool DISK_Initialise(DISK* disk, uint8_t driveNumber) {
    uint8_t driveType;
    uint16_t cylinders, sectors, heads;

    if (!x86_Disk_GetDriveParams(disk->id, &driveType, &cylinders, &sectors, &heads)) return false;
...
bool DISK_ReadSectors(DISK* disk, uint32_t lba, uint8_t sectors, uint8_t far* dataOut) {
    ...
    for (int i = 0; i < 3; i++) {
        if (x86_Disk_Read(disk->id, cylinder, head, sector, sectors, dataOut)) return true;
